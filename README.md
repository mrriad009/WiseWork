...
The **server** (when present) is a Node.js Express app intended to parse documents, optionally enrich with LinkedIn context, and call LLM providers (OpenAI, Google Gemini / Vertex AI, and others supported by your configuration).
...
- **AI / cloud:** `openai`, `@google/generative-ai`, `@google-cloud/vertexai`
...
- LLM / cloud keys as used by your implementation, for example: `OPENAI_API_KEY`, `GEMINI_API_KEY`, Vertex (`GOOGLE_PROJECT_ID`, `GOOGLE_LOCATION`, `GOOGLE_APPLICATION_CREDENTIALS`), or other providers your `index.ts` reads
