...
import { Upload, Loader2, ArrowLeft, Plus, Trash2, User, Search, Zap, Scale, Trophy, AlertTriangle, Sparkles, ChevronDown, TrendingUp, LayoutList, } from 'lucide-react';
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (!cancelled)
                    setSavedError('Could not load saved candidates.');
            }
            finally {
                if (!cancelled)
...
function SavedCandidatesPanel({ runs, loading, error, dbConfigured, }) {
    if (dbConfigured === false) {
        return (_jsxs("div", { className: "rounded-3xl border border-amber-200/80 bg-amber-50/90 px-6 py-10 text-center shadow-sm ring-1 ring-amber-100/80", children: [_jsx("p", { className: "font-display text-lg font-semibold text-amber-950", children: "Database not connected" }), _jsxs("p", { className: "mx-auto mt-3 max-w-lg text-sm leading-relaxed text-amber-900/90", children: ["Add your Neon connection string as", ' ', _jsx("code", { className: "rounded bg-white px-1.5 py-0.5 font-mono text-xs text-amber-950", children: "DATABASE_URL" }), " in", ' ', _jsx("code", { className: "rounded bg-white px-1.5 py-0.5 font-mono text-xs", children: "server/.env" }), ", run", ' ', _jsx("code", { className: "rounded bg-white px-1.5 py-0.5 font-mono text-xs", children: "npm run db:push" }), " in the server folder, then restart the API. Saved analyses will show here automatically."] })] }));
    }
    if (loading) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center rounded-3xl border border-stone-200 bg-white py-24", children: [_jsx(Loader2, { className: "h-10 w-10 animate-spin text-teal-700", "aria-hidden": true }), _jsx("p", { className: "mt-4 text-sm font-medium text-stone-600", children: "Loading saved candidates\u2026" })] }));
    }
    if (error) {
        return (_jsx("div", { className: "rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-900", role: "alert", children: error }));
    }
    if (runs.length === 0) {
        return (_jsxs("div", { className: "rounded-3xl border-2 border-dashed border-stone-200/90 bg-white/80 py-16 text-center md:py-20", children: [_jsx(LayoutList, { className: "mx-auto h-10 w-10 text-stone-300", strokeWidth: 1.25, "aria-hidden": true }), _jsx("p", { className: "mt-6 font-display text-lg font-semibold text-stone-800", children: "No saved candidates yet" }), _jsx("p", { className: "mt-2 text-sm text-stone-500", children: "Run an analysis on the \"Current analysis\" tab \u2014 each completed run is stored when Neon is configured." })] }));
    }
    return (_jsx("div", { className: "overflow-x-auto rounded-3xl border border-stone-200 bg-white shadow-md shadow-stone-900/[0.04] ring-1 ring-stone-200/60", children: _jsxs("table", { className: "w-full min-w-[720px] border-collapse text-left text-sm", children: [_jsx("thead", { children: _jsxs("tr", { className: "border-b border-stone-200 bg-stone-50/95", children: [_jsx("th", { className: "px-4 py-3.5 font-semibold text-stone-800", children: "Candidate" }), _jsx("th", { className: "px-4 py-3.5 font-semibold text-stone-800", children: "Score" }), _jsx("th", { className: "px-4 py-3.5 font-semibold text-stone-800", children: "Call" }), _jsx("th", { className: "whitespace-nowrap px-4 py-3.5 font-semibold text-stone-800", children: "Saved" }), _jsx("th", { className: "min-w-[240px] px-4 py-3.5 font-semibold text-stone-800", children: "Summary" })] }) }), _jsx("tbody", { children: runs.map((run) => {
                        ...
                        const summary = run.error ||
                            ...
                            '—';
                        return (_jsxs("tr", { className: "border-b border-stone-100 align-top last:border-0", children: [_jsxs("td", { className: "px-4 py-3.5", children: [_jsx("span", { className: "font-semibold text-stone-900", children: name }), run.file_name && (_jsx("span", { className: "mt-0.5 block truncate text-xs text-stone-500", title: run.file_name, children: run.file_name }))] }), _jsx("td", { className: "px-4 py-3.5 font-display text-lg font-bold tabular-nums text-teal-800", children: score ?? '—' }), _jsx("td", { className: "px-4 py-3.5", children: _jsx(DecisionPill, { decision: decision }) }), _jsx("td", { className: "whitespace-nowrap px-4 py-3.5 text-xs text-stone-500", children: new Date(run.created_at).toLocaleString() }), _jsx("td", { className: "max-w-md px-4 py-3.5 text-xs leading-relaxed text-stone-600", children: summary.length > 220 ? `${summary.slice(0, 220)}…` : summary })] }, run.id));
                    }) })] }) }));
}
const ResultCard = ({ result, rank, compactHeader, }) => {
    const [expanded, setExpanded] = useState(true);
    if (result.error) {
        return (_jsxs("div", { className: "rounded-3xl border border-red-200 bg-red-50/90 px-6 py-5 text-sm text-red-900 shadow-sm ring-1 ring-red-100", children: [_jsx("span", { className: "font-semibold", children: displayNameForResult(result, rank) }), _jsxs("span", { className: "text-red-800", children: [" \u2014 ", result.error] })] }));
    }
